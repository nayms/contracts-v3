...
library LibMarket {
    ...
    function _calculateTrade(
        ...
        MatchingOfferResult memory result;
        result.remainingBuyAmount = _buyAmount;
        result.remainingSellAmount = s.offers[bestOfferId].sellAmount;
    ...
    function _takeOfferSim(
        ...
        // check bounds and update balances
        // _checkBoundsAndUpdateBalancesSim(_offerId, _buyAmount, _sellAmount);
        MarketInfo memory marketOffer = s.offers[_offerId];
        marketOffer.sellAmount -= _buyAmount;
        marketOffer.buyAmount -= _sellAmount;
        /// Check to see if the fee schedule from the new order (`startTokenSale()`) is the initial offer fee schedule or not
        /// Use the initial offer fee schedule if it is, otherwise use the fee schedule from the original order placed
        {
            uint256 feeSchedule;
        ...
        // remaining amount
        if (marketOffer.sellAmount < LibConstants.DUST) {}
    }
